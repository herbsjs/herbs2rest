...
function generateEndpoints({ herbarium, server, convention = defaultConvention }) {
    ...
    for (let uc of herbarium.nodes.find({ type: herbarium.node.usecase })) {
        ...
        if (!Array.isArray(REST)) throw new Error(`'REST' metadata must be an array. It is not possible to generate a REST endpoint for usecase ${ucID}.`)
        ...
        for (let metadata of REST) {
            ...
            const usecase = uc.value
            if (!method) throw new Error(`'method' metadata is required. It is not possible to generate a REST endpoint for usecase ${ucID}.`)
            if (!path) throw new Error(`'path' metadata is required. It is not possible to generate a REST endpoint for usecase ${ucID}.`)
            if (!parameters) throw new Error(`'parameters' metadata is required. It is not possible to generate a REST endpoint for usecase ${ucID}.`)

            const paramsHandler = metadata.parametersHandler || parametersHandler
            if (!paramsHandler) throw new Error(`'parametersHandler' metadata is required. It is not possible to generate a REST endpoint for usecase ${ucID}.`)

            const authorizationHandler = metadata.authorizationHandler
            if (!authorizationHandler) throw new Error(`'authorizationHandler' metadata is required. It is not possible to generate a REST endpoint for usecase ${ucID}.`)

            const ctlr = metadata.controller
            if (!ctlr) throw new Error(`'controller' metadata is required. It is not possible to generate a REST endpoint for usecase ${ucID}.`)
            ...
            if (endpoint) throw new Error(`It is not possible to generate a REST endpoint for usecase '${ucID}'. There is already an endpoint for method ${method} and path ${path} generated by usecase '${endpoint.ucID}'.`)
            endpoints.push({ ucID, method, path })
